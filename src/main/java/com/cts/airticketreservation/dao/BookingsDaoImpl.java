...
public class BookingsDaoImpl implements BookingsDao {
	...
	@Override
	public ArrayList<Bookings> showBookings(String pnr) {
	...
	public void SendMail(String pnr_no, String cost) {
		...
		ArrayList<Bookings> Booking_list = new ArrayList<Bookings>();
		Booking_list = showBookings(pnr_no);

		final String query = "select p_email from passenger_details where p_pnr=? limit 1";
		Connection con = ConnectionHandler.getConnection();
		...
		try {
			...
			while (rs.next()) {
				...
				sendmail.send(email, pnr_no, Booking_list, cost);
			}
		} 
		catch (SQLException e) {
			e.printStackTrace();
