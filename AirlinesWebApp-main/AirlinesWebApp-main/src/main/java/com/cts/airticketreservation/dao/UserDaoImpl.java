...
import com.cts.airticketreservation.mail.SendEmail;
...
public class UserDaoImpl implements UserDao {
	...
	public String sendOTP(String email_id) {
        ...
        String OTP=null;
		Connection con = ConnectionHandler.getConnection();
		final String query = "select * from user_details where u_email_id=? limit 1";
		try {
			...
			if(rs.next())
			{
				OTP=sendmail.sendOTPmail(email_id);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public void sendCredentials(String email) {
		Connection con = ConnectionHandler.getConnection();
		final String query = "select u_username,u_password from user_details where u_email_id=? limit 1";
		try {
			...
			if(rs.next())
			{
				...
				sendmail.sendCredentialsMail(username,password,email);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
